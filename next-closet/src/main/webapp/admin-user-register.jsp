...
　<h2>新規会員登録</h2>
	...
	<div class="error-messages">
      <% for(String errorMessage : errorMessages) { %>
          <p><%= errorMessage %></p>
      <% } %>
      <% request.getSession().getAttribute("errorMessages"); %>
    </div>
	...
	<form action="AdminUserRegisterServlet" method="post">
		...
		<input type="text" id="postCode" name="userPostCode" 
		...
		<label>住所</label><textarea type="text" name="userAddress" placeholder="例) 〇〇市〇〇区〇丁目" required><%= request.getSession().getAttribute("address") != null ? request.getSession().getAttribute("address") : "" %></textarea><br>
